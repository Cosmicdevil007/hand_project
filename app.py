    ...
    while True:
        ...
        if results.multi_hand_landmarks is not None:
            for hand_landmarks, handedness in zip(results.multi_hand_landmarks,
                ...
                global latest_landmark
                latest_landmark = landmark_list[0]
                ...
                #print(pre_process_point_history)
...
def draw_landmarks(image, landmark_point):
    ...
    for index, landmark in enumerate(landmark_point):
        ...
        if index == 3:  # Thumb: First joint
            cv.circle(image, (landmark[0], landmark[1]), 5, (255, 255, 255),-1)
            cv.circle(image, (landmark[0], landmark[1]), 5, (0, 0, 0), 1)
        if index == 4:  # Thumb: Tip of finger
...
def draw_info_text(image, brect, handedness, hand_sign_text,
    ...
    cv.putText(image, info_text, (brect[0] + 5, brect[1] - 5),
               cv.FONT_HERSHEY_SIMPLEX, 0.9, (285, 285, 285), 1, cv.LINE_AA)

    if finger_gesture_text != "":
        cv.putText(image, "Finger Gesture:" + finger_gesture_text, (10, 60),
                   cv.FONT_HERSHEY_SIMPLEX, 1.0, (0, 0, 0), 4, cv.LINE_AA)
        cv.putText(image, "Finger Gesture:" + finger_gesture_text, (10, 60),
                   cv.FONT_HERSHEY_SIMPLEX, 1.0, (285, 285, 285), 2,
                   cv.LINE_AA)
...
def draw_info(image, fps, mode, number):
    ...
    cv.putText(image, "FPS:" + str(fps), (10, 30), cv.FONT_HERSHEY_SIMPLEX,
               1.0, (285, 285, 285), 2, cv.LINE_AA)

    mode_string = ['Logging Key Point', 'Logging Point History']
    if 1 <= mode <= 2:
        cv.putText(image, "MODE:" + mode_string[mode - 1], (10, 90),
                   cv.FONT_HERSHEY_SIMPLEX, 0.6, (285, 285, 285), 1,
                   cv.LINE_AA)
        if 0 <= number <= 9:
            cv.putText(image, "NUM:" + str(number), (10, 110),
                       cv.FONT_HERSHEY_SIMPLEX, 0.6, (255, 285, 285), 1,
                       cv.LINE_AA)
    return image
...
def generate_hand_keypoint_frames():
    args = get_args()
